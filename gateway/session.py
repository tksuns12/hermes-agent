...
class SessionStore:
    ...
    def get_or_create_session(
        ...
        if self._db and db_end_session_id:
            try:
                self._db.end_session(db_end_session_id, "session_reset", user_id=tenant)
            except Exception as e:
    ...
    def switch_session(self, session_key: str, target_session_id: str) -> Optional[SessionEntry]:
        ...
        if self._db and db_end_session_id:
            try:
                self._db.end_session(db_end_session_id, "session_switch", user_id=tenant)
            except Exception as e:
                logger.debug("Session DB end_session failed: %s", e)

        if self._db:
            try:
                self._db.reopen_session(target_session_id, user_id=tenant)
            except Exception as e:
