...
class SessionDB:
    ...
    def end_session(self, session_id: str, end_reason: str, user_id: str = None) -> None:
        ...
        def _do(conn):
            conn.execute(
                ...
                (time.time(), end_reason, session_id, tenant),
            )
        self._execute_write(_do)

    def reopen_session(self, session_id: str, user_id: str = None) -> None:
        """Clear ended_at/end_reason so a session can be resumed."""
        tenant = self._tenant(user_id)

        def _do(conn):
            conn.execute(
                "UPDATE sessions SET ended_at = NULL, end_reason = NULL WHERE id = ? AND user_id = ?",
                (session_id, tenant),
            )
        self._execute_write(_do)
    ...
    def replace_messages(
        ...
        def _do(conn):
            ...
            conn.execute(
                ...
                "WHERE id = ? AND user_id = ?",
                (total_messages, total_tool_calls, session_id, tenant),
            )
