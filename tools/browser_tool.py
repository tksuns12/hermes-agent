...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, str]:
    ...
    force_local = _is_local_sidecar_key(normalized_task)
    ...
    if not force_local:
        _ensure_cdp_supervisor(runtime_key)
...
def cleanup_browser(task_id: Optional[str] = None) -> None:
    ...
    if _is_local_sidecar_key(task_label):
        session_keys = [runtime_key]
        bare_task_id, _, _ = resolve_runtime_key(task_label[: -len(_LOCAL_SUFFIX)], tenant)
    else:
        session_keys = [runtime_key]
        sidecar_runtime_key, _, _ = resolve_runtime_key(f"{task_label}{_LOCAL_SUFFIX}", tenant)
        with _cleanup_lock:
            if sidecar_runtime_key in _active_sessions:
                session_keys.append(sidecar_runtime_key)
        bare_task_id = runtime_key
    ...
    if not _is_local_sidecar_key(task_label):
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    _stop_cdp_supervisor(runtime_key)
