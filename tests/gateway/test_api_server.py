...
class TestResponsesEndpoint:
    ...
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_responses_normalizes_input_file_parts(self, adapter):
        mock_result = {"final_response": "done", "messages": [], "api_calls": 1}
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert "note.txt" in user_message
            assert "hello file" in user_message
            assert ".hermes" not in user_message

    @pytest.mark.asyncio
    async def test_responses_rejects_missing_input_file(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                resp = await cli.post(
                    ...
                    json={
                        "input": [
                            {
                                "role": "user",
                                "content": [{"type": "input_file", "file_id": "file_missing"}],
                            }
                        ]
                    },
                )
    ...
    @pytest.mark.asyncio
    async def test_responses_rejects_foreign_tenant_input_file(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            owned = await cli.post(
                "/v1/files",
                        ...
                        "input": [
                            {
                                ...
                                "content": [
                                    ...
                                    {"type": "input_file", "file_id": owned_id},
                                ],
                            }
                        ]
                    },
