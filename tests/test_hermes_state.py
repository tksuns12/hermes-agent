...
class TestSchemaInit:
    ...
    def test_reconciliation_adds_missing_columns(self, tmp_path):
        ...
        migrated_db = SessionDB(db_path=db_path)
        msg_cols = {r[1] for r in migrated_db._conn.execute("PRAGMA table_info(messages)").fetchall()}
        assert "reasoning_content" in msg_cols

        cursor = migrated_db._conn.execute(
            "SELECT role, content, reasoning, reasoning_content, reasoning_details, codex_reasoning_items "
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        for table_name, declared_cols in expected.items():
            live_cols = {
                r[1]
                for r in db._conn.execute(f'PRAGMA table_info("{table_name}")').fetchall()
            }
