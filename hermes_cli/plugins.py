...
def discover_plugins(force: bool = False, *, force_reload: bool = False) -> None:
    """Discover and load all plugins.

    Default behavior is idempotent. Pass ``force=True`` to rescan plugin
    manifests and reload state in the current process. ``force_reload`` is
    kept as a backwards-compatible alias for callers that need a fresh scan.
    """
    get_plugin_manager().discover_and_load(force=force or force_reload)
